...
def _load_and_train(df: pd.DataFrame, cache_key: str | None = None) -> dict[str, Any]:
    ...
    if cache_path and os.path.exists(cache_path):
        try:
            ...
            print(f"[mcp_server] Local cache load failed ({e}), retraining from scratch.")
    ...
    # ── Save cache locally to VPS disk ──────────────────────────────────────
    if cache_path and cache_key:
        try:
            ...
            print(f"[mcp_server] Saved model cache locally to VPS: {cache_path}")
        except Exception as e:
