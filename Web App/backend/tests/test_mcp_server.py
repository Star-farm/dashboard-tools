...
# ── 2. New Test Cases Added to Increase Coverage ──────────────────────────────

def test_gcs_blob_name():
    assert mcp_server._gcs_blob_name("test_key") == "model-cache/test_key.joblib"


@patch("mcp_server.GCS_CACHE_BUCKET", "test-bucket")
@patch("google.cloud.storage.Client")
def test_try_download_cache_from_gcs_success(mock_storage_client):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        local_path = os.path.join(tmpdir, "cache.joblib")
        result = mcp_server._try_download_cache_from_gcs("dummy_key", local_path)
...
@patch("mcp_server.GCS_CACHE_BUCKET", "test-bucket")
@patch("google.cloud.storage.Client")
def test_try_download_cache_from_gcs_failure(mock_storage_client):
    mock_storage_client.side_effect = Exception("GCS Connection Error")
    result = mcp_server._try_download_cache_from_gcs("dummy_key", "dummy_path")
    assert result is False


@patch("mcp_server.GCS_CACHE_BUCKET", "test-bucket")
@patch("google.cloud.storage.Client")
def test_try_upload_cache_to_gcs_failure(mock_storage_client):
    mock_storage_client.side_effect = Exception("Upload Denied")
    # Verify that the function does not crash the application when upload fails
    mcp_server._try_upload_cache_to_gcs("dummy_key", "dummy_path")
...
def test_load_and_train_local_cache_corrupted(mock_load):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("mcp_server.MODEL_CACHE_DIR", tmpdir):
            ...
            cache_file_path = os.path.join(tmpdir, "dummy_key.joblib")
