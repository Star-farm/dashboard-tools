...
def test_bundle_round_trip_uses_one_packaged_state():
    ...
    with tempfile.TemporaryDirectory() as cache_dir:
        ...
        loaded, source = load_bundle(fingerprint, cache_dir)
...
def test_gcs_download_failure_does_not_trigger_training():
    with tempfile.TemporaryDirectory() as cache_dir:
        with patch("app.ml.artifacts._download_from_gcs", return_value=False):
            with pytest.raises(RuntimeError, match="Model artifact not found"):
                load_bundle("missing", cache_dir, "bucket")
