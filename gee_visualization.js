...
var resetInspector = function() {
  ...
  valueLabel.style().set({ color: '#7f8c8d', fontSize: '14px', fontWeight: 'normal',
...
var legendTitleLabel = ui.Label({ style: { fontWeight: 'bold', fontSize: '14px', margin: '0 0 8px 0' } });
var legendSubLabel   = ui.Label({ style: { fontSize: '12px', margin: '0 0 8px 0' } });
...
var crossfade = function(fromIdx, toIdx, step) {
  ...
  state.mapLayers.forEach(function(layer, i) {
...
var selectYear = function(index, snap) {
  ...
  if (snap) {
    state.mapLayers.forEach(function(layer, i) { layer.setOpacity(i === index ? 1.0 : 0.0); });
  } else {
...
// ====================================================================
// 8. MODE SWITCHER & WORKSPACE RESET
// ====================================================================
...
var switchMode = function(modeName) {
  ...
  Map.layers().reset();
  Map.addLayer(table.style({ color: '#ffffff00', width: 0, fillColor: '00000000' }), {}, 'Polygon Boundaries');
  Map.layers().add(highlightLayer);

  state.mapLayers = cfg.data.map(function(item, i) {
    var layer = ui.Map.Layer(item.image, cfg.vis, 'Year ' + item.year, true);
  ...
  table.filterBounds(clickPoint).geometry().evaluate(function(geom) {
    ...
    highlightLayer.setEeObject(
        table.filter(ee.Filter.bounds(clickPoint))
            .style({ color: 'yellow', width: 2.5, fillColor: '00000000' })
    );
